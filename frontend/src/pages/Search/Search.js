...
const Search = () => {
    ...
    const [initCriteria, setInitCriteria] = useState("places")

    const setPlacesActive = () => {
        isActive.places = true
        isActive.hotels = false
        isActive.flights = false
        isActive.poi = false
        setActive({...isActive, places: true})
        setActive({...isActive, hotels: false})
        setActive({...isActive, flights: false})
        setActive({...isActive, poi: false})

        setInitCriteria("places")
    }

    const setHotelsActive = () => {
        isActive.places = false
        isActive.hotels = true
        isActive.flights = false
        isActive.poi = false
        setActive({...isActive, places: false})
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '5%' }}>
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', width: '100%', marginBottom: '1%'}}>
                <Button  
                    sx={{ backgroundColor: isActive.places ? 'rgba(207, 125, 43, .31)' : 'rgba(0,0,0,0)', marginLeft: 2, marginRight: 2 }} 
                    onClick={() => setPlacesActive()}
                    fullWidth
                >Places</Button>
